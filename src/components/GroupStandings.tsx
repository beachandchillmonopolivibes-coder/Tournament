...
const GroupStandings = ({ group, isEditingEnabled = false }: GroupStandingsProps) => {
  const [editingTeam, setEditingTeam] = useState<{ id: string, name: string, players: string[] } | null>(null);
  const { currentTournament, updateTeamDetails } = useTournamentStore();

  const handleSaveTeam = async (newName: string, newPlayers: string[]) => {
    if (editingTeam && currentTournament) {
      await updateTeamDetails(currentTournament.id, group.id, editingTeam.id, newName, newPlayers);
    }
  };
  ...
  return (
    <>
                ...
                <tr key={team.id} className="hover:bg-[#1f2833] transition-colors">
                  <td className="py-3 px-4 font-bold text-white flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isEditingEnabled && (
                        <button
                          onClick={() => setEditingTeam({ id: team.id, name: team.name, players: team.players })}
                          className="p-1 hover:bg-white/10 rounded transition-colors text-gray-400 hover:text-neon-blue"
      ...
      {editingTeam && <EditTeamModal isOpen={!!editingTeam} onClose={() => setEditingTeam(null)} onSave={handleSaveTeam} currentName={editingTeam.name} currentPlayers={editingTeam.players} />}
    </>
  );
};
